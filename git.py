...
# ── Tag format ──
# Unified bracket format used in commit messages and audit logs:
#   [mcpp:ver=1,user=alice,project=infra,task=build-auth,step=3,flags=l,sid=a3f7b2c,notes=some note]
# Per-file lines:
#   path/to/file.py [mcpp:ver=1,user=alice,...]
...
# Per-file line: filename followed by [mcpp:...] tag
FILE_LINE_PATTERN = re.compile(
    r"^(?P<name>\S+)\s+\[mcpp:(?P<pairs>[^\]]+)\]$"
)

KNOWN_KEYS = {"ver", "user", "project", "task", "step", "flags", "sid", "notes"}


def _needs_quoting(value: str) -> bool:
    """Return True if a value needs double-quoting in tag format."""
    return "," in value or "]" in value


def _quote(value: str) -> str:
    """Quote a value if it contains , or ]."""
    if _needs_quoting(value):
        return f'"{value}"'
    return value


def _parse_pairs(pairs_str: str) -> dict[str, str]:
    """Parse key=value pairs from inside [mcpp:...].

    Handles quoted values (for notes containing commas).
    notes is always parsed as everything after 'notes=' until end.
    """
    kv: dict[str, str] = {}
    # Handle notes specially — grab everything after notes= to end
    notes_idx = pairs_str.find("notes=")
    remainder = pairs_str
    if notes_idx >= 0:
        notes_val = pairs_str[notes_idx + 6:]
        # Strip surrounding quotes if present
        if notes_val.startswith('"') and notes_val.endswith('"'):
            notes_val = notes_val[1:-1]
        kv["notes"] = notes_val
        remainder = pairs_str[:notes_idx].rstrip(",")
...
class McppTag:
    ...
    def format(self) -> str:
        ...
        if self.notes is not None:
            parts.append(f"notes={_quote(self.notes)}")
        return f"[mcpp:{','.join(parts)}]"


def parse_tag(message: str) -> Optional[McppTag]:
    """Extract the commit-level McppTag from a commit message, or None.

    Uses the last [mcpp:...] match — build_message always appends the
    commit-level tag as the final element.
    """
    matches = list(TAG_PATTERN.finditer(message))
    if not matches:
        return None
    m = matches[-1]
    kv = _parse_pairs(m.group("pairs"))
    step_val = None
    if "step" in kv:
        try:
...
def parse_file_lines(message: str) -> list[tuple[str, McppTag]]:
    """Extract per-file metadata lines from a commit message body.

    Returns list of (filename, McppTag) tuples.
    """
    entries = []
    for line in message.splitlines():
        ...
        if m:
            ...
            entries.append((m.group("name"), tag))
    return entries


def build_message(message: str, tag: McppTag, file_tags: Optional[list[tuple[str, McppTag]]] = None) -> str:
    """Build a commit message with optional per-file lines and the mcpp tag appended.

    file_tags: list of (filename, McppTag) tuples for per-file metadata.
    tag: commit-level summary tag (last line).
    """
    parts = [message]
    if file_tags:
        parts.append("")  # blank line before file lines
        for filename, ftag in file_tags:
            parts.append(f"{filename} {ftag.format()}")
...
_log = logging.getLogger("mcpp_git")
...
def file_owner(cwd: str | Path, filepath: str) -> Optional[str]:
    ...
    for name, ftag in parse_file_lines(message):
        if name == filepath and ftag.user:
            return ftag.user
...
def grep_files(
    ...
    """Search tracked files for a pattern using grep -r.

    Returns list of {path, line, text} dicts.
    Uses git ls-files to scope to tracked files only, piped to grep.
    """
    # Build grep args
    grep_args = ["-rn", "--color=never"]
    if ignore_case:
        grep_args.append("-i")
    if context_lines > 0:
    ...
    grep_args.extend(["--exclude-dir=.git", "--exclude-dir=node_modules",
                       "--exclude-dir=__pycache__", "--exclude-dir=.worktrees"])
    grep_args.append(pattern)
    grep_args.append(".")

    _log.debug("running: grep %s", " ".join(grep_args))
    t0 = time.monotonic()
    result = subprocess.run(
        ["grep"] + grep_args,
        cwd=str(cwd),
    ...
    elapsed = time.monotonic() - t0
    _log.debug("grep completed in %.3fs (rc=%d)", elapsed, result.returncode)

    if result.returncode not in (0, 1):  # 1 = no matches
        raise GitError(f"grep failed: {result.stderr.strip()}", result.returncode, result.stderr.strip())

    entries = []
    for line in result.stdout.splitlines():
        if len(entries) >= max_results:
            break
        # Format: ./path/file.js:123:matched text
        parts = line.split(":", 2)
        if len(parts) >= 3:
            ...
            try:
                ...
                continue
            text = parts[2]
            entries.append({"path": filepath, "line": lineno, "text": text})
    return entries
...
def find_files(
    ...
    """Find tracked files by name pattern or extension.

    Uses git ls-files for speed (no filesystem walk).
    """
    result = _run(["ls-files"], cwd)
    ...
    if pattern:
        import fnmatch
        files = [f for f in files if fnmatch.fnmatch(f.split("/")[-1], pattern) or fnmatch.fnmatch(f, pattern)]
